...
# import ate_spyder.widgets.actions_on.ctrl.mqtt_client as mqtt
...
from ate_spyder.widgets.actions_on.ctrl.sequencer import (
    Sequencer,
)  # TODO: is it possible to get the necessary info from ATE.projectdatabase.Sequence
from ate_spyder.widgets.actions_on.ctrl.barprogress import Barprogress

try:
    from labml_adjutancy.misc import showstdf

    has_misc = True
except ImportError:
    print(f"import showstdf is going wrong {ImportError}")
    has_misc = False
...
class Control(QtWidgets.QDialog):
    ...
    command = (
        {  # see https://github.com/Semi-ATE/Semi-ATE/blob/master/docs/project/interfacedefinitions/testapp_interface.md
            "load": {
                "type": "cmd",
                "command": "load",
    ...
    def __init__(self, project_info):
        ...
        self.sendtopic = "ate/SCT-81-1F/TestApp/"
        ...
        self.logfilename = ""
        self.last_testprogramm = []
        # connect to mqtt and intial (needs function mqtt_receive)
        self.mqttclient = mqttclient
        self.mqtt.mqtt_add()
        self.computername = (
            os.environ.get("COMPUTERNAME") if self.mqttclient.broker == "127.0.0.1" else self.mqttclient.broker
        )
        # connected to GUI-functions:
        self.adjustUI()
        self.cycle = 0
        self.topinstname = ""
        self._state = "init"
    ...
    def __call__(self, project_info):
        self.logger.debug(f"__call__({project_info.project_name})")
        if project_info == "":
            return
        self.test_program_name = "_".join(
            (
                ...
                self.project_info.run_group,
                self.project_info.run_flow,
            )
        )
        if self.last_testprogramm != [
            ...
            for app in self.gui.newMenu:
                if hasattr(app, "libname") and app.libname != "":
                    ...
                    if (
                        self.saveconfigdata is not None
                        and len(self.saveconfigdata["menu"][app.name]) > 2
                        and app.instance is not None
                    ):
                        self.set_Geometry(
                            app.name,
                            app.instance.gui,
                            self.saveconfigdata["menu"][app.name][2],
                        )
                index += 1
        self.sequencer.load(
            self.project_info.project_directory, self.test_program_name
        )  # get the latest parameters for shmoos
        self.last_testprogramm = [
            self.project_info.project_directory,
            self.test_program_name,
        ]
    ...
    def _createSeqMenu(self):
        ...
        # TODO: not necessary if trigger from Semi-ATE to Semi-control available
        print("TODO: Menue Update Sequencer, not necessary if trigger from Semi-ATE to Semi-control available")
        updateSeqAction = QtWidgets.QAction(self.gui)
        updateSeqAction.setText("Update Sequencer")
        self.gui.Gsequencer.addAction(updateSeqAction)
        updateSeqAction.triggered.connect(lambda: self.guiclicked("updatesequencer"))
    ...
    def mqttReiveMyname(self, topic, msg):
        ...
        if type(msg) is dict and self.SemiCtrlinstName in msg:  # received a message from semictrl
            ...
            if msg["type"] == "cmd" and msg["cmd"] == "menu":  # get command to extend Menu
                ...
                for addmenu in msg["payload"]:
                    ...
                    pindex += 1
                    found = False
                    self.logger.debug(f"     add {addmenu} - {self.gui.newMenu}")
                    if pindex == 0 and addmenu not in self.gui.menuBar.titles:  # add new Menu Title
                        ...
                        self.logger.debug(f"     add new Menu Title {addmenu}")
                        mindex = pindex
                        found = True
                        ...
                        if found:
                            ...
                            try:
                                ...
                                self.logger.error(f"    Coudn't load {msg['payload']}[{addmenu}]  {ex}")
                            ...
                            self.gui.newMenu[-1].triggered.connect(
                                lambda checked, index=len(self.gui.newMenu) - 1: self.extendedbarClicked(index)
                            )
                            self.logger.debug(
    ...
    def extendedbarClicked(self, index):
        self.logger.debug(f"extendedbarClicked {index}")
        self.logger.debug(f"     name = {self.gui.newMenu[index].name}")
        self.logger.debug(f"     libname = {self.gui.newMenu[index].libname}")
        self.logger.debug(f"     lib = {self.gui.newMenu[index].lib}")
        self.logger.debug(f"     subtopic = {self.gui.newMenu[index].subtopic}")
        self.logger.debug(f"     topinstname = {self.topinstname}")
        if self.gui.newMenu[index].isChecked():
            ...
            if not hasattr(self.gui.newMenu[index], "lib") or self.gui.newMenu[index].lib is None:
                self.logger.error(f" Gui for {name} not exist, please update your adjutancy lib")
            ...
            self.gui.newMenu[index].instance = self.gui.newMenu[index].lib.Gui(self, name)  # start Gui
    ...
    def onChangeTabs(self, index):
        ...
        self.logger.info(f" show {path}/{self.test_program_name} has misc: {has_misc}")
        if index == 1 and has_misc:
            lot = "no_lot"
            if Path(f"{path}/{self.test_program_name}.stdf").is_file():
                import Semi_ATE.STDF as stdf

                self.logger.info(f"    {stdf.__version__}  ")
                self.logger.info(f"    showstdf.showStdf({path}/, {self.test_program_name}.stdf, {lot}, result_h5")
                self.dataframe = showstdf.showStdf(
                    path + "/",
                    f"{self.test_program_name}.stdf",
                    lot,
                    "result_h5",
                )
                self.logger.info("             create stdf data done ")
                self.dataframe.show(self.gui.resulttable)
            else:
                print(f"{path}/{self.test_program_name}.stdf not found")
    ...
    def guiclicked(self, cmd):
        ...
        if cmd == "clear":
            ...
            self.logger.info(self.project_info.parent.parent.__class__)
            self.logger.info(dir(self.project_info.parent.parent))
        elif cmd == "mqtt":
            ...
            # if (not last or not self.gui.actionSequencer_Parameters.isChecked()) and self.command['SetParameter']['parameters'] != []:
            if (
    ...
    def state(self, value):
        ...
        if type(value) == tuple:
            ...
            value = value[0]
        if (
            value in ["idle", "testing"] and self.command["SetParameter"]["parameters"] != []
        ...
        if value.find("error") > -1:  # wo wird das gesetzt?? TODO: change!!
            ...
            value == "idle" and oldstate in ("reset", "config", "testing") and self.gui.CBstopauto.isChecked()
        ):  # testprogramm runs through
    ...
    def _change_status_display(self, msg, extendmsg=""):
        ...
        style = (
            self._states[msg][1] if msg in self._states else "color: rgb(255, 255, 255);background-color: transparent;"
        )
    ...
    # configuration :
    def openconfig(self):
        """Open the configuration file c:users/$USER"""  # TODO: use config...
        ...
        try:
            self.logger.debug(f"open last settings: {settings_file}")
            ...
            self.logger.debug(f"   now create menu from {settings_file}")
            if "menu" in data:
                menu = {}
                menu["type"] = "cmd"
                menu["cmd"] = "menu"
                ...
                for app in self.gui.newMenu:
                    if app.libname != "":
                        ...
                        self.extendedbarClicked(index)
                        if len(data["menu"][app.name]) > 2 and app.instance is not None:
                            self.set_Geometry(
                                app.name,
                                app.instance.gui,
                                data["menu"][app.name][2],
                            )
            ...
            self.logger.debug(f"{self.configfile}: {ex} not found or not ok -> use rest of default config")
    ...
    def saveconfig(self, button=None):
        ...
        self.logger.debug(f"write {settings_file}")
    ...
    def close(self, event=None):
        print("Semi-Control closed window")
        self.saveconfig()
        for app in self.gui.newMenu:
            if app.instance is not None:
                app.instance.gui.close()
        if self.mqttsave is not None:
            self.mqttsave.close()
