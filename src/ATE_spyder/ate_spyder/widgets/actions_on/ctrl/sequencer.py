...
class Sequencer(object):
    ...
    def load(self, path, test_program_name):
        ...
        #     self.remove_paraBox()
        self.parent.logger.debug(f'      Sequencer load path = {path} name= {test_program_name}')
        # open the jsonfile for the test_program_name and collect all information about the shmoo parameter
        tests = JsonDict(os.path.join(path, 'definitions', 'sequence', f'sequence{test_program_name}.json'))
        choicesParameter = {}
        ...
        for test in tests.contents.values(key='test'):
            ...
            for index in range(0, len(mytest)):
                is_selected = True if mytest[index].definition.is_selected else is_selected
            myparameter.append(is_selected)
            for key in input_paramters.keys():
                if input_paramters[key].Shmoo and key not in choicesParameter:
        ...
        self.parent.logger.debug(f'      Sequencer choices parameter: {choicesParameter}')
        parameters = self._parameters if len(self.parameters) == 0 or self.parameters[0][1] == '' else self.parameters
    ...
    def paraBoxValues(self):
        ...
        try:
            mybox.value = mybox.values[mybox.index] if len(mybox.values) > 0 else ''
        except Exception:
            mybox.value = ''
        ...
        for test in table:
            for parameter in table[test]:
                ...
                if parameter not in parameters:
                    ...
                    if not selectable:
                        combobox.setItemIcon(combobox.count()-1, qta.icon('ei.remove-sign', color='white', scale_factor=1.0, color_active='orange'))
    ...
    def create_paraBox(self):
        ...
        pb = self.paraBox[-1]
        pb.setGeometry(QtCore.QRect(BOX_OFFSET_X, BOX_OFFSET_Y + index*(BOX_HEIGHT), BOX_WIDTH, BOX_HEIGHT))
        pb.show()
        pb.check = QtWidgets.QCheckBox(pb)
        pb.check.setGeometry(QtCore.QRect(INNERBLOCK_OFFSET_X, INNERBLOCK_OFFSET_Y, pb.check.geometry().height(), pb.check.geometry().height()))
        pb.check.show()
        pb.cb = QtWidgets.QComboBox(pb)
        pb.cb.setGeometry(QtCore.QRect(pb.check.geometry().x()+pb.check.geometry().width()-8, INNERBLOCK_OFFSET_Y, CB_WIDTH, CB_HEIGHT))
        pb.cb.show()
        pb.edit = QtWidgets.QLineEdit(pb)
        pb.edit.setGeometry(QtCore.QRect(pb.cb.geometry().x(), pb.cb.geometry().x() + INNERBLOCK_OFFSET_Y, CB_WIDTH, CB_HEIGHT))
        pb.edit.setToolTip("Syntax:\n 4:6  -> loop from 4 til 6 step 1\n 4.5:6.0:0.1 -> loop from 4.5 til 6.0 step 0.1\n 4.6,6.5,8.3 -> seperate values 4.6,4.6,6.5,8.3/n ")
        pb.edit.editingFinished.connect(lambda: self.editfinish(index))
        pb.edit.show()
        pb.lvalue = QtWidgets.QLabel(pb)
        pb.lvalue.setGeometry(QtCore.QRect(pb.edit.geometry().x()+pb.edit.geometry().width()+INNERBLOCK_OFFSET_X, pb.edit.geometry().y(), 30, CB_HEIGHT))
        pb.lvalue.setText('')
        pb.lvalue.show()
        ...
        if index > 0:
            ...
            pb.rb_identAction.triggered.connect(lambda: self.move_paraBox(index, 1))
            pb.rb_marchAction = QtWidgets.QAction(pb)
            pb.rb_marchAction.setText("march")
            pb.addAction(pb.rb_marchAction)
            pb.rb_marchAction.triggered.connect(lambda: self.move_paraBox(index, -1))
        ...
        if index > 0:
            self.paraBox[-2].rb_addAction.setVisible(False)    # only last parabox has the add item
        if index > 1:
    ...
    def move_paraBox(self, index, inc):
        ...
        pb.level += inc
        pb.setGeometry(pb.geometry().x()+inc*10, pb.geometry().y(), pb.geometry().width(), pb.geometry().height())
        new_width = pb.geometry().x()+pb.geometry().width() + 2
        self.parent.logger.debug(f'      width = {self.gui.geometry().width()}, new width= {new_width}')
        if self.gui.geometry().width() < new_width:
            self.gui.setGeometry(self.gui.geometry().x(), self.gui.geometry().y(), new_width, self.gui.geometry().height())     # TODO: isn't running :-(
    ...
    def parameters(self):
        ...
        for index in range(0, len(self.paraBox)):
            pb = self.paraBox[index]
            values.append([pb.check.isChecked(), pb.cb.currentText(), pb.edit.text(), pb.level])
        return(values)
    ...
    def getnext(self, start=True):
        ...
        self.parent.logger.debug(f'     self.choicesParameter = {self.choicesParameter}')
        if self.finish:
            self.setfirstvalue()
            self.finish = False
            return 'finish', True
        for index in range(len(parameters), 0, -1):
            ...
            if not foundnewvalue:                                        # inc the loop
                ...
                if self.paraBox[index-1].last:
                    ...
                    incloop = index-2
                foundnewvalue = True
            elif foundnewvalue and not self.paraBox[index-1].last:          # get old value
            ...
            if value == '':
                continue
            if type(value) == str:
                self.parent.logger.error(f'     parameter, from {name}:={value} is not valid ')
                result = []
                break
            for test in self.choicesParameter:
                if name in self.choicesParameter[test] and self.choicesParameter[test][0]:
                    result.append({"parametername": f"{test}.{name}", "value": f"{value}"})
        last = self.finish if foundnewvalue else True
        if incloop > -1:
            self.getnextvalue(incloop)
        elif incloop == -1:
            self.finish = True
        self.parent.logger.debug(f'     Sequencer getnext: {result}, {self.finish}')
        #last = self.finish
        #if self.finish:
